...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self.bottomImageView addSubview:self.downloadBtn];
...
- (void)layoutLandscape:(Boolean)landscapeFullscreen {
    ...
    if (landscapeFullscreen) {
        ...
        [self.topImageView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.leading.trailing.equalTo(self);
            make.top.equalTo(self.mas_top).offset(0);
            make.height.mas_equalTo(70 + window.safeAreaInsets.top);
        }];
        [self.backBtn mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.leading.equalTo(self.topImageView.mas_leading).offset(5);
...
- (void)makeSubViewsConstraints {
    ...
    [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        // sandstalk
        make.centerY.equalTo(self.backBtn.mas_centerY).offset(-2);
    ...
    [self.resolutionBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo(kIconSize);
        make.width.mas_greaterThanOrEqualTo(45);
        make.trailing.equalTo(self.bottomImageView.mas_trailing).offset(-5);
        make.centerY.equalTo(self.startBtn.mas_centerY);
    }];
    
    CGFloat offset = -(kIconSize + 5);
    if (!self.nextBtn.hidden){
        [self.nextBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.trailing.equalTo(self.bottomImageView.mas_trailing).offset(offset);
            make.centerY.equalTo(self.startBtn.mas_centerY);
            make.width.height.mas_equalTo(kIconSize);
        }];
        offset -= kIconSize;
...
- (void)downloadBtnClick:(UIButton *)sender {
    sender.selected = !sender.selected;
    if ([self.delegate respondsToSelector:@selector(controlViewDidSelectDownload:)]){
        [self.delegate controlViewDidSelectDownload:self];
    }
}
...
- (void)setDisableDownloadBtn:(BOOL)disableDownloadBtn{
    _disableDownloadBtn = disableDownloadBtn;
    self.downloadBtn.hidden = disableDownloadBtn;
}
...
- (void)setOrientationLandscapeConstraint {
    // sandstalk
    if(!self.enableFullscreen) return;
    self.fullScreen = YES;
...
- (void)setOrientationPortraitConstraint {
    ...
    // sandstalk
    self.fullScreenBtn.hidden   = !self.enableFullscreen;
    self.resolutionBtn.hidden   = YES;
    self.moreBtn.hidden         = YES;
    self.captureBtn.hidden      = YES;
    self.danmakuBtn.hidden      = YES;
...
- (UIButton *)downloadBtn {
    if (!_downloadBtn) {
        ...
        [_downloadBtn setImage:SuperPlayerImage(@"offline_download") forState:UIControlStateNormal];
...
- (void)setDownloadBtnState:(BOOL)isShow {
    self.disableDownloadBtn = !isShow;
}
